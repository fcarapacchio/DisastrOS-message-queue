...
#define DESCRIPTOR_BUFFER_SIZE (MAX_NUM_DESCRIPTORS * DESCRIPTOR_SIZE)

#define DESCRIPTORPTR_SIZE sizeof(DescriptorPtr)
#define DESCRIPTORPTR_BUFFER_SIZE (MAX_NUM_DESCRIPTORS * DESCRIPTORPTR_SIZE)
...
DescriptorPtr* DescriptorPtr_alloc(Descriptor* descriptor) {
  DescriptorPtr* d = (DescriptorPtr*) PoolAllocator_getBlock(&_descriptor_ptr_allocator);
  if (!d)
    return 0;
...
void DescriptorList_print(ListHead* l) {
  ...
  while (aux) {
    Descriptor* d = (Descriptor*) aux;
    printf("(fd: %d, rid: %d)", d->fd, d->resource->id);
    if (aux->next)
      printf(", ");
    aux = aux->next;
  }
